...
from reportgen_cancer_ACOR import generate_summary_chart
...
class SOCHA_ACOR:
    ...
    def _rank_asc_with_random_ties(self, values):
        ...
        order = np.argsort(values[perm], kind='mergesort')
    ...
    def _gen_X(self, dist_mean, dist_rank, nl, n_of_points, q, k, xi):
        ...
        for l in range(n_of_points):
            ...
            for m in range(num_dims - 1):
                if available.size == 0: return None
                sub = r_dist_mean[available, m:]
                if sub.shape[0] == 0 or sub.shape[1] == 0: return None
                dis = np.apply_along_axis(self._euc_dist, 1, sub)
                if np.sum(dis) == 0.0: return None
                
                if available.size > 1:
                    p_choice = np.power(dis, 4.0)
                    p_choice = p_choice / p_choice.sum()
                    choice = available[np.random.choice(len(available), p=p_choice)]
                else:
                    choice = available[0]
            ...
            dist_sd = np.array([np.sum(np.abs(r_dist_mean[nl[j], i] - r_dist_mean[j, i])) / (k - 1) for i in range(num_dims)])
...
def evaluate_acor(X_train, X_test, y_train, y_test, n_runs=50):
    ...
    num_weights = FNN.get_num_weights()
    ...
    for run in range(n_runs):
        ...
        acor = SOCHA_ACOR(
            obj_func=obj_func, dim=num_weights, n_ants=2, n_samples=148,
            q=0.95, xi=0.98, max_iter=100, patience=15, seed=42 + run
        )
...
if __name__ == "__main__":
    print("SOCHA-ACOR for Cancer Classification")
    print("=" * 60)
    ...
    # --- CHANGE: Replaced old file saving with the new, organized logic ---
    ...
    pkl_filename = f'cancer_socha_acor_model_{timestamp}.pkl'
