"""
Preprocessing module for Dr. PROBEN disease classifiers.

Handles data normalization and feature expansion for diabetes, heart, and cancer datasets.
Provides both single-input and batch preprocessing with automatic label detection,
missing value handling, and PROBEN1 format encoding support.

Key Components:
- BasePreprocessor: Abstract base class with common validation utilities
- DiabetesPreprocessor: Min-max scaling (0-1) for 8 features
- HeartPreprocessor: Feature expansion (13→35) and normalization
- CancerPreprocessor: Scale normalization (1-10 → 0-1) for 9 features
- expand_heart_features(): Converts raw heart features to 35 encoded features

Usage:
    preprocessor = get_preprocessor('diabetes')
    X, y_true, msgs = preprocessor.preprocess_batch_matrix(data_array)
"""
